...
async def run_orchestrator_loop():
    ...
    if not settings.ORCHESTRATOR_ENABLED:
        print("⚠️ Orchestrator is disabled in config")
        return
    
    _orchestrator_running = True
    cycle_count = 0
    
    print(f"🤖 Orchestrator started - running every {settings.ORCHESTRATOR_INTERVAL_SECONDS}s")
    ...
    while _orchestrator_running:
        ...
        try:
            ...
            async with AsyncSessionLocal() as db:
                ...
                if actions:
                    print(f"🔄 Cycle {cycle_count}: {len(actions)} actions to execute")
                    ...
                    print(f"✅ Cycle {cycle_count}: {execution_result['success_count']} succeeded, {execution_result['error_count']} failed")
                else:
                    print(f"⏸️ Cycle {cycle_count}: No actions needed, system healthy")
            ...
            print(f"❌ Orchestrator cycle {cycle_count} error: {e}")
