...
async def spike_scheduler_task():
    ...
    if not settings.AUTO_PUBLISHER_ENABLED:
        print("⚠️ SPIKE Scheduler deshabilitado (AUTO_PUBLISHER_ENABLED=False)")
        return
    
    print("🔄 Iniciando SPIKE Scheduler...")
    ...
    while True:
        try:
            ...
            print(f"❌ Error en SPIKE Scheduler: {e}")
            log_job_event(
                job_id=None,
...
async def run_auto_spike_detection(
    ...
    print(f"📊 Analizando {len(adsets)} adsets activos...")
    ...
    for adset in adsets:
        try:
            ...
            print(f"⚠️ Error detectando spike en adset {adset['id']}: {e}")
    
    # Contar spikes detectados
    spikes_detected = sum(1 for r in spike_results if r.spike_detected)
    
    print(f"🎯 {spikes_detected} spikes detectados de {len(spike_results)} adsets")
...
def start_scheduler_background():
    ...
    if not settings.AUTO_PUBLISHER_ENABLED:
        print("⚠️ SPIKE Scheduler deshabilitado")
        return
    
    print("🚀 Iniciando SPIKE Scheduler en background...")
