...
class OptimizationActionStatus(str, enum.Enum):
    """Optimization action status enumeration."""
    SUGGESTED = "suggested"  # Created by optimizer, awaiting review
    PENDING = "pending"  # Approved, waiting execution
...
class OptimizationActionModel(Base):
    """
    Optimization actions generated by the Meta Optimization Loop.
    
    Stores suggested and executed optimization actions like scale_up, scale_down,
    pause, reallocate. Supports both 'suggest' mode (manual review) and 'auto' mode
    (automatic execution with guard rails).
    """
    __tablename__ = "optimization_actions"
    
    # Primary fields
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid4)
    action_id = Column(String(100), unique=True, nullable=False, index=True)  # Unique identifier
    
    # Target identification
    campaign_id = Column(UUID(as_uuid=True), ForeignKey("meta_campaigns.id"), nullable=True, index=True)
    adset_id = Column(UUID(as_uuid=True), ForeignKey("meta_adsets.id"), nullable=True, index=True)
    ad_id = Column(UUID(as_uuid=True), ForeignKey("meta_ads.id"), nullable=True, index=True)
    
    # Action details
    action_type = Column(String(50), nullable=False, index=True)  # scale_up, scale_down, pause, resume, reallocate
    target_level = Column(String(20), nullable=False)  # campaign, adset, ad
    target_id = Column(String(100), nullable=False)  # META_CAMPAIGN_xxx, META_ADSET_xxx, META_AD_xxx
    
    # Action parameters
    amount_pct = Column(Float, nullable=True)  # Budget change percentage (-100 to +200)
    amount_usd = Column(Float, nullable=True)  # Absolute budget change in USD
    new_budget_usd = Column(Float, nullable=True)  # New budget value
    old_budget_usd = Column(Float, nullable=True)  # Previous budget value
    
    # Rationale
    reason = Column(String(255), nullable=False)  # roas_below_threshold, high_performance, etc.
    reason_details = Column(Text, nullable=True)  # Detailed explanation
    confidence = Column(Float, nullable=False)  # 0.0 to 1.0
    
    # Supporting metrics
    roas_value = Column(Float, nullable=True)  # ROAS that triggered action
    spend_usd = Column(Float, nullable=True)  # Current spend
    revenue_usd = Column(Float, nullable=True)  # Current revenue
    conversions = Column(Integer, nullable=True)  # Current conversions
    impressions = Column(Integer, nullable=True)  # Current impressions
    
    # Status and execution
    status = Column(
        Enum(OptimizationActionStatus),
        nullable=False,
        default=OptimizationActionStatus.SUGGESTED,
        index=True
    )
    created_by = Column(String(100), nullable=False)  # optimizer, user_email, system
    approved_by = Column(String(100), nullable=True)  # User who approved (if manual)
    executed_by = Column(String(100), nullable=True)  # User/system who executed
    
    # Timing
    created_at = Column(DateTime, default=datetime.utcnow, nullable=False, index=True)
    approved_at = Column(DateTime, nullable=True)
    executed_at = Column(DateTime, nullable=True)
    expires_at = Column(DateTime, nullable=True)  # Suggestions expire after N hours
    
    # Execution results
    execution_error = Column(Text, nullable=True)
    execution_result = Column(JSON, nullable=True)  # API response, details
    meta_response = Column(JSON, nullable=True)  # Response from Meta Ads API
    
    # Guard rails and safety
    safety_score = Column(Float, nullable=True)  # 0.0 to 1.0, higher = safer
    guard_rails_passed = Column(Integer, nullable=False, default=1)  # 1 = passed, 0 = failed
    guard_rails_details = Column(JSON, nullable=True)  # Which guard rails were checked
    
    # Reallocation specific (for budget reallocation actions)
    reallocation_plan = Column(JSON, nullable=True)  # Full budget reallocation plan
    affected_ad_ids = Column(JSON, nullable=True)  # List of ad IDs affected by reallocation
    
    # Metadata
    params = Column(JSON, nullable=True)  # Additional parameters
    tags = Column(JSON, nullable=True)  # Tags for filtering (e.g., ["high_priority", "auto_approved"])
    notes = Column(Text, nullable=True)  # Human notes/comments
    
    # Ledger reference
    ledger_event_id = Column(UUID(as_uuid=True), nullable=True)  # Reference to EventLogModel
    ...
    def __repr__(self):
        return f"<OptimizationAction(id={self.action_id}, type={self.action_type}, status={self.status}, target={self.target_id})>"
